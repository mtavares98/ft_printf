...
/*   Updated: 2022/03/13 17:48:51 by mtavares         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_format(const char *str, int i, va_list *args)
{
	...
	if (str[i] == 'u')
		return (ft_putllunbr_base((t_llu)va_arg(*args, t_u), DEC, 10, 0));
	if (str[i] == 'x')
		return (ft_putllunbr_base((t_llu)va_arg(*args, t_u), HEXLO, 16, 0));
	if (str[i] == 'X')
		return (ft_putllunbr_base((t_llu)va_arg(*args, t_u), HEXUP, 16, 0));
	if (str[i] == '%')
		return (ft_putstr(0, '%', CHAR));
	if (str[i] == 'p')
		return (write(1, "0x", 2) + ft_putllunbr_base(va_arg(*args, t_llu),
				HEXLO, 16, 0));
	return (0);
}
